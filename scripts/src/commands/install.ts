...
import { detect, loadAllTemplates, loadMcpTemplate, paths } from "../utils.js";
...
async function selectScope(
  ...
  if (cliScope) {
    ...
    // 检查模板是否支持该 scope
    const supported = template.metadata.supportedScopes || [
      "user",
      "project",
      "local",
    ];
    if (!supported.includes(cliScope as Scope)) {
      throw new Error(
        `模板 ${template.metadata.name} 不支持 ${cliScope} scope`
      );
    }
  ...
  return await clack.select({
    ...
    options: supported.map((s) => {
      ...
      return {
        ...
        hint: `${detect.getRecommendReason(s)}${
          isRecommended ? " ⭐ 推荐" : ""
        }`,
      };
    }),
...
async function performInstall(
  ...
  try {
    ...
    // 合并或替换
    const final =
      strategy === "merge" ? merge(existing, template.config) : template.config;
    ...
    if (template.metadata.claudeMd) {
      ...
      const claudeMdTemplatePath = join(
        paths.templates(),
        template.metadata.claudeMd
      );
...
async function installMcpConfig(
  scope: Scope,
  mcpTemplate: { mcpServers: Record<string, any> }
): Promise<string> {
  let mcpPath: string;
...
async function installMcpServersViaCli(
  ...
  if (failedServers.length === 0) {
    ...
    clack.log.warn(
      `部分安装成功: ${successCount} 成功, ${failedServers.length} 失败`
    );
