...
function listPresets(prefix: string): string[] {
  return readdirSync(ROOT).filter((f) => f.startsWith(prefix) && f.endsWith('.json'))
}
...
describe('atoms stay within their dimension', () => {
  ...
  function optionsOf(file: string): Record<string, unknown> {
    ...
    const options = preset['compilerOptions']
    if (options === undefined || options === null) return {}
    if (typeof options !== 'object' || Array.isArray(options)) {
      throw new Error(`${file}: compilerOptions must be an object`)
    }
