...
describe('composeConfig - fixture lint', () => {
  ...
  it('invalid/imports-newline-after.ts 应报 import-x/newline-after-import（imports 模块开启时）', async () => {
    const result = await lintFile(
      path.join(fixturesDir, 'invalid/imports-newline-after.ts'),
      composeConfig({
        typescript: { tsconfigRootDir: import.meta.dirname },
        imports: true,
      }),
  ...
  describe('选项控制', () => {
    it('unicorn: false 时 unicorn 规则不触发', async () => {
      const result = await lintFile(
        path.join(fixturesDir, 'invalid/unicorn-node-protocol.ts'),
        composeConfig({
          typescript: { tsconfigRootDir: import.meta.dirname },
          unicorn: false,
        }),
    ...
    it('stylistic: false 时 @stylistic/quotes 不触发', async () => {
      const result = await lintFile(
        path.join(fixturesDir, 'invalid/stylistic-quotes.ts'),
        composeConfig({
          typescript: { tsconfigRootDir: import.meta.dirname },
          stylistic: false,
        }),
